...
const appName = `runtime-smoke-restart-invalid-manifest-shape-${uniqueSuffix}`;
const runtimePort = 9940 + Math.floor(Math.random() * 40);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restart-invalid-manifest-shape-smoke-"));
...
try {
  ...
  if (restartResult.code === 0) {
    throw new Error(
      `Expected restart to fail when manifest contract values are invalid.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected restart failure to clearly explain semantic manifest validation issue.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
