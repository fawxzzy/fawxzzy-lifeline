...
const appName = `runtime-smoke-restore-blocked-foreign-owner-${uniqueSuffix}`;
...
async function waitForPortOwner(expectedPid) {
  ...
  throw new Error(`Timed out waiting for blocked port owner pid ${expectedPid}`);
}

async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restore-blocked-owner-smoke-"));
  ...
  const manifestForBlockedRestore = manifestRaw
    .replace(/^name: .*$/m, `name: ${appName}`)
    .replace(/^port: .*$/m, `port: ${runtimePort}`)
    .replace(/^  restartPolicy: .*$/m, "  restartPolicy: never");

  await writeFile(tempManifestPath, manifestForBlockedRestore, "utf8");
...
try {
  ...
  if (!startedState.childPid || !isPidAlive(startedState.childPid)) {
    throw new Error("Expected running state with live managed child before forcing blocked restore state");
  }
  ...
  if (restoreResult.code !== 0) {
    throw new Error(
      `Expected restore to complete while preserving blocked foreign owner boundary.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }

  if (
    !restoreResult.stdout.includes(
      `Skipping ${appName}: last known status is blocked; not restorable as running.`,
    )
  ) {
    throw new Error(
      `Expected restore output to explicitly skip blocked foreign-owner app.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }

  if (restoreResult.stdout.includes(`Restored ${appName} with supervisor pid`)) {
    throw new Error(
      `Expected restore not to relaunch blocked app while foreign process owns managed port.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
  ...
  if (!statusAfterRestore.stdout.includes(`App ${appName} is blocked.`)) {
    throw new Error(
      `Expected status to remain blocked after restore under foreign port ownership.\nstdout:\n${statusAfterRestore.stdout}\nstderr:\n${statusAfterRestore.stderr}`,
    );
  }

  if (!statusAfterRestore.stdout.includes(`- portOwner: pid ${foreignPid}`)) {
    throw new Error(
      `Expected blocked status to continue reporting foreign port owner pid ${foreignPid} after restore.\nstdout:\n${statusAfterRestore.stdout}\nstderr:\n${statusAfterRestore.stderr}`,
    );
  }
  ...
  if (persistedAfterRestore.lastKnownStatus !== "blocked") {
    throw new Error(
      `Expected persisted runtime status blocked after restore with foreign owner, got ${persistedAfterRestore.lastKnownStatus}`,
    );
  }
  ...
  if (persistedAfterRestore.childPid && isPidAlive(persistedAfterRestore.childPid)) {
    throw new Error(
      `Expected persisted state child pid to remain non-running after restore, found live pid ${persistedAfterRestore.childPid}`,
    );
  }
} catch (error) {
  await cleanup();
