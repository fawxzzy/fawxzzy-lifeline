...
const appName = `runtime-smoke-restart-playbook-invalid-persisted-${uniqueSuffix}`;
const runtimePort = 9880 + Math.floor(Math.random() * 40);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restart-playbook-invalid-smoke-"));
...
try {
  ...
  if (restartResult.code === 0) {
    throw new Error(
      `Expected restart to fail when persisted playbook path is invalid.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }

  if (!restartResult.stderr.includes("Playbook export directory does not exist")) {
    throw new Error(
      `Expected restart failure to clearly explain invalid persisted playbook path.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
  ...
  if (persistedAfterRestart.playbookPath !== invalidPersistedPath) {
    throw new Error(
      `Expected persisted corrupted playbook path to remain ${invalidPersistedPath}, found ${persistedAfterRestart.playbookPath}`,
    );
  }
