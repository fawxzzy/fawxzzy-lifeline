...
const appName = `runtime-smoke-restart-missing-env-file-${uniqueSuffix}`;
const runtimePort = 9910 + Math.floor(Math.random() * 40);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restart-missing-env-file-smoke-"));
...
try {
  ...
  if (restartResult.code === 0) {
    throw new Error(
      `Expected restart to fail when env.file is missing.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected restart failure to clearly explain missing env.file path.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
