...
const appName = `runtime-smoke-restore-stopped-supervisor-kill-${uniqueSuffix}`;
...
async function waitForStoppedWithHistory() {
  ...
  const latestStatus = await run(["status", appName], { allowFailure: true });
  throw new Error(
    `Timed out waiting for stopped status output with persisted history after supervisor kill.\nstdout:\n${latestStatus.stdout}\nstderr:\n${latestStatus.stderr}`,
  );
}

async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(
    path.join(tmpdir(), "lifeline-runtime-restore-stopped-supervisor-kill-smoke-"),
  );
...
try {
  ...
  if (persistedBeforeRestore.lastKnownStatus === "running") {
    throw new Error("Expected supervisor kill to converge away from running before restore");
  }
