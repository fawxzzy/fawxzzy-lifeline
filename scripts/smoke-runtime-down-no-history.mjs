...
const appName = `runtime-smoke-down-no-history-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
...
if (await fileExists(appLogPath)) {
  throw new Error(`Expected no log file before down command, found ${appLogPath}`);
}

const processesBefore = await listProcessesForApp(appName);
if (processesBefore.length > 0) {
  throw new Error(
    `Expected no app-related processes before down command, found:\n${processesBefore.join("\n")}`,
  );
}
...
if (downResult.code === 0) {
  throw new Error(
    `Expected down command to fail for never-started app, got success.\nstdout:\n${downResult.stdout}\nstderr:\n${downResult.stderr}`,
  );
}

if (!downResult.stderr.includes(`No runtime state found for app ${appName}.`)) {
  throw new Error(
    `Expected explicit no-runtime-state down message for ${appName}.\nstdout:\n${downResult.stdout}\nstderr:\n${downResult.stderr}`,
  );
}

await assertNoPersistedState(appName, "after down command");

if (await fileExists(appLogPath)) {
  throw new Error(`Expected down command not to create ${appLogPath}, but file exists`);
}
...
if (statusResult.code === 0) {
  throw new Error(
    `Expected status to fail for never-started app after down command.\nstdout:\n${statusResult.stdout}\nstderr:\n${statusResult.stderr}`,
  );
}

if (!statusResult.stderr.includes(`No runtime state found for app ${appName}.`)) {
  throw new Error(
    `Expected status to report no runtime state after down command.\nstdout:\n${statusResult.stdout}\nstderr:\n${statusResult.stderr}`,
  );
}
