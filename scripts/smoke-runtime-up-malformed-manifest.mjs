...
const appName = `runtime-smoke-up-malformed-manifest-${uniqueSuffix}`;
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(
    path.join(tmpdir(), "lifeline-runtime-up-malformed-manifest-smoke-"),
  );
  ...
  await writeFile(tempManifestPath, malformedManifest, "utf8");
...
try {
  ...
  if (upResult.code === 0) {
    throw new Error(
      `Expected up to fail when manifest YAML is malformed.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected up failure to clearly explain malformed manifest YAML.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
