...
const appName = `runtime-smoke-up-malformed-env-file-${uniqueSuffix}`;
const runtimePort = 9920 + Math.floor(Math.random() * 60);

let manifestPath;
let malformedEnvFile;
let tempRootDir;
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(
    path.join(tmpdir(), "lifeline-runtime-up-malformed-env-file-smoke-"),
  );
  const tempFixtureDir = path.join(tempRootDir, "runtime-smoke-app");

  await cp(fixtureDir, tempFixtureDir, { recursive: true });

  malformedEnvFile = path.join(tempFixtureDir, ".env.runtime.malformed");
  await writeFile(malformedEnvFile, "PORT\nGREETING=hello\n", "utf8");

  const tempManifestPath = path.join(tempFixtureDir, fixtureManifest);
  const manifestRaw = await readFile(tempManifestPath, "utf8");
  const manifestWithMalformedEnvFile = manifestRaw
    .replace(/^name: .*$/m, `name: ${appName}`)
    .replace(/^port: .*$/m, `port: ${runtimePort}`)
    .replace(/^  file: .*$/m, "  file: .env.runtime.malformed");

  await writeFile(tempManifestPath, manifestWithMalformedEnvFile, "utf8");
...
try {
  ...
  if (upResult.code === 0) {
    throw new Error(
      `Expected up to fail when env.file is malformed.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }

  const combinedOutput = `${upResult.stdout}\n${upResult.stderr}`;
  if (
    !combinedOutput.toLowerCase().includes("invalid env line") &&
    !combinedOutput.includes(malformedEnvFile)
  ) {
    throw new Error(
      `Expected up failure to clearly explain malformed env.file contents.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
  ...
  if (persistedAfterUp) {
    ...
    if (persistedAfterUp.supervisorPid && isPidAlive(persistedAfterUp.supervisorPid)) {
      throw new Error(
