...
const appName = `runtime-smoke-restore-missing-env-file-${uniqueSuffix}`;
const runtimePort = 9240 + Math.floor(Math.random() * 500);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restore-missing-env-file-smoke-"));
  ...
  const manifestForRestoreMissingEnv = manifestRaw
  ...
  await writeFile(tempManifestPath, manifestForRestoreMissingEnv, "utf8");
...
try {
  ...
  await rm(resolvedEnvFile, { force: true });

  const restoreResult = await run(["restore"], { allowFailure: true });
  if (restoreResult.code === 0) {
    throw new Error(
      `Expected restore to fail when env.file is missing.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }

  const combinedOutput = `${restoreResult.stdout}\n${restoreResult.stderr}`;
  if (
    !combinedOutput.toLowerCase().includes("could not read env file") &&
    !combinedOutput.includes(resolvedEnvFile)
  ) {
    throw new Error(
      `Expected restore failure to clearly explain missing env.file path.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
