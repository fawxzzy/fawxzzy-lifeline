...
async function verifyDarwinDefaultSelection() {
  ...
  assert(
    inspection.mechanism === "launchd-agent",
    `Expected launchd-agent mechanism for darwin, got ${inspection.mechanism}.`,
  );
  assert(
...
async function verifyAixDefaultSelection() {
  const backend = resolveStartupBackend({ platform: "aix" });
  const inspection = await backend.inspect();

  assert(
    backend.id === "aix-inittab",
    `Expected aix-inittab backend id for aix, got ${backend.id}.`,
  );
  assert(
    inspection.mechanism === "aix-inittab",
    `Expected aix-inittab mechanism for aix, got ${inspection.mechanism}.`,
  );
  assert(
    ["installed", "not-installed", "unsupported"].includes(inspection.status),
    `Expected aix inspection status to be installed|not-installed|unsupported, got ${inspection.status}.`,
  );
}
...
async function verifyUnsupportedDefaultSelection(platform) {
  ...
  assert(
    ...
    `Expected unsupported backend for ${platform} to report unsupported.`,
  );
  assert(
    inspection.status === "unsupported",
    `Expected unsupported status for ${platform}, got ${inspection.status}.`,
  );
  assert(
...
async function verifyInjectedBackendSelection() {
  ...
  assert(
    backend.id === "deterministic-fake",
    `Expected injected backend selection, got ${backend.id}.`,
  );
  assert(
    inspection.supported === true,
    "Expected injected backend inspect to be returned as-is.",
  );
  assert(
    inspection.mechanism === "deterministic-fake",
    `Expected fake mechanism, got ${inspection.mechanism}.`,
  );
}
...
async function verifyRegistrySelectionAndFallback() {
  ...
  assert(
    selected.id === "registry-backend",
    `Expected registry backend to be selected, got ${selected.id}.`,
  );
  assert(
    selectedInspection.mechanism === "registry-backend",
    "Expected registry inspection result.",
  );
  ...
  assert(
    ...
    `Expected unsupported fallback backend, got ${fallback.id}.`,
  );
  assert(
    fallbackInspection.status === "unsupported",
    `Expected unsupported fallback status, got ${fallbackInspection.status}.`,
  );
}
...
async function main() {
  ...
  await verifyAixDefaultSelection();
  await verifyUnsupportedDefaultSelection("sunos");
...
main().catch((error) => {
  ...
  console.error(
    `Deterministic startup backend selection verification failed: ${message}`,
  );
  process.exitCode = 1;
});
